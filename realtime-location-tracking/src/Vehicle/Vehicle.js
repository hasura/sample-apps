...
function Vehicle(props) {
  ...
  useInterval(() => {
    if (startTracking) {
      ...
      .then(response => {
        /* 
          added "-1" so it doesn't try to access the 57th position
            in the locationData array
          it doesn't exist and it will throw an error
        */
        const newLocation = locationData.length - 1 === locationId ? 0 : locationId + 1;
        ...
        { startTracking ?
          [
            <ApolloConsumer key={ '1' }>
              {client => (
                <Subscription subscription={ GET_USERS } variables={{ vehicleId }}>
                  {({ loading, error, data }) => {
